...
app.get('/api/:alienName', (req, res) =>{
    ...
    if(aliens[aliensName]){ //if the input is inside of the aliens object then send back info on that
        res.json(aliens[aliensName])
    } else{
        res.json(aliens['humans']) // if its not, its prolly a human so just default to sending back info on humans
    }
});

app.listen(process.env.PORT || PORT, () => {
    console.log('server is running')
}); // heroku you can pick a better port if you want otherwise just use port 8000